...
async def check_anniversaries():
    ...
    for guild in client.guilds:
        ...
        if "sent_log" not in settings:
            settings["sent_log"] = {}

        if guild_id not in settings["sent_log"]:
            settings["sent_log"][guild_id] = {}

        if today_key not in settings["sent_log"][guild_id]:
            settings["sent_log"][guild_id][today_key] = []
        ...
        for member in guild.members:
            ...
            if joined.month == now.month and joined.day == now.day:
                ...
                if years >= 1:
                    if member.id in settings["sent_log"][guild_id][today_key]:
                        continue
...
Loyalty recognized. Respect earned."""
                    ...
                    settings["sent_log"][guild_id][today_key].append(member.id)
